...
def setup_base_system():
    ...
    execute_command(
        f'sgdisk --clear '
        f'--new=1:0:+1G --typecode=1:ef00 --change-name=1:"EFI" '
        f'--new=2:0:0   --typecode=2:8300 --change-name=2:"ROOT" '
        f'{Drive}'
    )

    print("\n---------------Formatting partitions---------------------")
    execute_command(f"mkfs.vfat -F 32 {boot_part}")
    execute_command(f"mkfs.ext4 -F {root_part}")

    print("\n---------------Mounting partitions---------------------")
    execute_command(f"mount {root_part} /mnt")
    # WICHTIG: EFI unter /boot/efi, NICHT direkt unter /boot
    # Dadurch landet vmlinuz-linux in /boot (auf der root-partition)
    # und grub-mkconfig findet ihn problemlos
    execute_command(f"mkdir -p /mnt/boot/efi")
    execute_command(f"mount {boot_part} /mnt/boot/efi")

    print("\n---------------Installing base system---------------------")
    # openssl + ca-certificates nötig damit pacman-key später nicht fehlschlägt
    # dhcpcd als Fallback-Netzwerk
    execute_command(
        "basestrap /mnt "
        "base base-devel runit elogind-runit "
        "linux linux-firmware linux-headers "
        "networkmanager networkmanager-runit dhcpcd "
        "openssl ca-certificates "
        "grub efibootmgr "           # <-- schon hier installieren, vor chroot
        "nano sudo python git curl wget"
    )
...
# GRUB installieren
# --efi-directory zeigt jetzt auf /boot/efi (nur die EFI-Partition)
# der Kernel liegt in /boot (root-partition) -> grub-mkconfig findet ihn
grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=GRUB --recheck

# Sicherstellen dass grub-mkconfig den Kernel wirklich sieht
ls /boot/vmlinuz* || (echo "FEHLER: Kernel nicht in /boot gefunden!" && exit 1)
...
def finalize_installation():
    ...
    # Erst /boot/efi, dann /mnt – Reihenfolge wichtig!
    execute_command("umount /mnt/boot/efi")
