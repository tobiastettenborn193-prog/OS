...
# Pakete die zuerst gebraucht werden (Mirrors, Keys, AUR-Build)
before_packages = [
    "git",
    "base-devel",
    "reflector",
    "artix-archlinux-support",  # Arch-Repos aktivieren
]

# Wayland / Niri Stack
wayland_packages = [
    "niri",
    "xwayland-satellite",       # X11 Apps unter Wayland
    "waybar",
    "fuzzel",
    ...
    "qt5-wayland",              # Qt-Support für Wayland
    "qt6-wayland",
    "polkit",                   # Nativ Polkit
    "polkit-gnome",             # Polkit Agent
    "gnome-keyring",            # Secrets portal
    "elogind",                  # Seat-Management (Artix systemd-Ersatz)
    "elogind-runit",
]
...
# Entwicklung & Microcontroller
dev_packages = [
    "code",                 # VSCode
    "python",
    "python-pip",
    "python-psutil",        # Oft nützlich für Waybar-Scripts
    "uv",
    "rust",
    "cargo",
    "go",
...
misc_packages = [
    "flatpak",
    "unzip",
    "wl-clipboard",             # Clipboard im Terminal
    "grim",                     # Screenshots
    "slurp",                    # Screenshot-Auswahl
    "libnotify",
    "tailscale-runit",
    "obsidian",
    "bluez",
    ...
    "nemo",                     # GUI Dateimanager
    "nemo-fileroller",          # Archiv-Support für Nemo
]

# AUR Pakete (via paru/smart_install)
aur_packages = [
    "zen-browser-bin",
    "librewolf-bin",
    "vesktop-bin",
    ...
    "zellij",
    "oh-my-zsh-git",
    "python-pywal",             # Erstmal nur installieren, Postscripts später
]
...
def set_temporary_sudo():
    """Gewährt dem wheel-User vorübergehend NOPASSWD-Rechte, um Prompts zu vermeiden."""
    print("\n=== Temporäre Sudo-Rechte setzen ===")
    ...
    print("-> Sudo Passwort-Eingabe deaktiviert.")
...
def update_mirrors():
    ...
    try:
        ...
        run("pacman -Syy")
        print("-> Mirrors aktualisiert.")
    except Exception as e:
        print(f"-> Warnung: Mirrors konnten nicht aktualisiert werden: {e}")
...
def enable_arch_repos():
    ...
    try:
        ...
        if not any(line.strip() == "[extra]" for line in lines):
            ...
            print("-> Arch-Repos waren bereits aktiv.")
    except Exception as e:
        print(f"-> Warnung: {e}")
...
def setup_system():
    print("\n=== System-Grundeinstellungen & Permissions ===")
    ...
    # System-Services aktivieren (runit)
    services = ["NetworkManager", "bluetoothd", "tailscaled", "elogind"]
    for svc in services:
        try:
            run(f"ln -sf /etc/runit/sv/{svc} /etc/runit/runsvdir/default/{svc}")
        except Exception:
            print(f"Warnung: Service {svc} konnte nicht verlinkt werden.")

    print("-> System-Grundeinstellungen gesetzt.")


def setup_pipewire():
    print("\n=== Pipewire (runit User-Services) ===")
    ...
    for svc in ["pipewire", "pipewire-pulse", "wireplumber"]:
        ...
        run_script = f"#!/bin/sh\nexec {svc}\n"
        write_file(f"{sv_dir}/run", run_script)
    ...
    # Autostart in .zprofile einrichten, damit die User-Services nach dem Login starten
    zprofile_path = f"{HOME}/.zprofile"
    autostart_cmd = "export SVDIR=~/.runit/runsvdir\nrunsvdir -P ~/.runit/runsvdir &\n"
    with open(zprofile_path, "a") as f:
        f.write(f"\n# Start Runit User Services\n{autostart_cmd}")
    run(f"chown {USER}:{USER} {zprofile_path}")

    print("-> Pipewire User-Services eingerichtet.")
...
user = "greeter"
"""
    write_file("/etc/greetd/config.toml", greetd_config)

    # greetd runit service
    sv_dir = "/etc/runit/sv/greetd"
    os.makedirs(sv_dir, exist_ok=True)
    write_file(f"{sv_dir}/run", "#!/bin/sh\nexec greetd\n")
    run(f"chmod +x {sv_dir}/run")
    run(f"ln -sf {sv_dir} /etc/runit/runsvdir/default/greetd")

    # getty auf tty1 deaktivieren
    run("rm -f /etc/runit/runsvdir/default/agetty-tty1")

    print("-> greetd eingerichtet.")
...
def setup_firewall():
    ...
    try:
        ...
        run("ufw allow 53317/tcp")   # LocalSend
        run("ufw allow 53317/udp")
        run("ufw logging on")
        run("ufw --force enable")
        sv_dir = "/etc/runit/sv/ufw"
        os.makedirs(sv_dir, exist_ok=True)
        write_file(f"{sv_dir}/run", "#!/bin/sh\nexec ufw start\n")
        run(f"chmod +x {sv_dir}/run")
        run(f"ln -sf {sv_dir} /etc/runit/runsvdir/default/ufw")
        print("-> Firewall aktiviert.")
    except Exception as e:
        print(f"-> Warnung UFW: {e}")
...
def setup_opsec():
    ...
    try:
        write_file("/etc/sysctl.d/99-opsec.conf", sysctl)
        run("sysctl --system")
...
def clone_dotfiles():
    ...
    print(f"-> Repo nach {REPO_DIR} geklont.")
...
"""
    write_file(f"{config_dir}/alacritty.toml", alacritty_toml, USER)
    run(f"chown -R {USER}:{USER} {config_dir}")
...
gtk-xft-rgba=rgb
"""
    write_file(f"{gtk_dir}/settings.ini", settings, USER)
...
error_symbol = "[❯](red)"
"""
    write_file(f"{HOME}/.config/starship.toml", starship_toml, USER)
...
def setup_zsh():
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            for plugin_url, plugin_name in [
                ("https://github.com/zsh-users/zsh-autosuggestions", "zsh-autosuggestions"),
                ("https://github.com/zsh-users/zsh-syntax-highlighting.git", "zsh-syntax-highlighting"),
            ]:
                run_as_user(f"git clone {plugin_url} {HOME}/.oh-my-zsh/custom/plugins/{plugin_name}")
    except Exception as e:
...
def setup_wallpapers():
    print("\n=== Wallpapers ===")
    walls_src = f"{REPO_DIR}/wallpapers"
    walls_dst = f"{HOME}/.wallpapers"

    if os.path.exists(walls_src):
        run(f"mkdir -p {walls_dst}")
        run(f"cp -r {walls_src}/. {walls_dst}/")
        run(f"chown -R {USER}:{USER} {walls_dst}")
        print("-> Wallpapers kopiert (Pywal wurde absichtlich übersprungen).")
    else:
        print("-> Warnung: wallpapers/ nicht im Repo.")


def setup_mullvad():
    print("\n=== Mullvad VPN ===")
    try:
        run(f"ln -sf /etc/runit/sv/mullvad-daemon /etc/runit/runsvdir/default/mullvad-daemon")
        print("-> Mullvad daemon aktiviert.")
    except Exception as e:
        print(f"-> Warnung: {e}")
...
"""
    ...
    write_file(f"{bin_dir}/volume.sh", vol_script)
    write_file(f"{bin_dir}/powermenu.sh", power_script)
...
def main():
    try:
        ...
        print("-> Sudo Passwort wieder aktiv.")

    print("\n" + "=" * 50)
    if failed_packages:
        print(f"Fehlgeschlagen: {', '.join(failed_packages)}")
    else:
        print("Alles erfolgreich! Reboot -> greetd begrüßt dich.")
    print("=" * 50)
